...
function deleteFileModPage(that, b_no) {
    var url = "/board/file/delete/" + articleNo;
    deleteFile(url, that);
}
...
function getFiles(b_no) {
    $.getJSON("/board/file/list/" + b_no, function (list) {
        if (list.length === 0) {
...
function getFileInfo(fullName) {
    ...
    if (checkImageType(fullName)) {
        imgSrc = "/board/file/display?file_name=" + fullName; // 썸네일 이미지 링크
        uuidFileName = fullName.substr(14);
        var originalImg = fullName.substr(0, 12) + fullName.substr(14);
        // 원본 이미지 요청 링크
        originalFileUrl = "/board/file/display?file_name=" + originalImg;
    } else {
        imgSrc = "/resources/upload/files/file-icon.png"; // 파일 아이콘 이미지 링크
        uuidFileName = fullName.substr(12);
        // 파일 다운로드 요청 링크
        originalFileUrl = "/board/file/display?file_name=" + fullName;
    }
